...
<div class="container-fluid px-3 px-md-4">
    <div class="card card-custom">
        <div class="card-header">
            <h4 class="card-title">تعديل المستخدم: {{ $profile->name }}</h4>
        </div>
        <div class="card-body p-4">
            ...
            <form action="{{ route('profile.update', $profile->id) }}" method='post'>
                ...
                <div class="mb-3">
                    ...
                    <input class="form_control_custom" type="text" id="name" name="name" value="{{ old('name', $profile->name) }}" required>
                </div>

                <div class="mb-3">
                    <label class="form-label" for="email">البريد الإلكتروني:</label>
                    <input class="form_control_custom" type="email" id="email" name="email" value="{{ old('email', $profile->email) }}" required>
                </div>

                <div class="mb-3">
                    <label class="form-label" for="password">كلمة المرور:</label>
                    <input class="form_control_custom" type="password" id="password" name="password" placeholder="اتركها فارغة لعدم التغيير">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select class="form_control_custom" name="role" id="role_selector">
                        @foreach ($roles as $role)
                            <option value="{{ $role->name }}" @selected(old('role', $profile->roles->pluck('name')->first()) == $role->name)>
                                {{ $role->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div id="confirmer_settings_wrapper" style="display: none;">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="confirmer_payment_type" id="payment_type_selector" class="form_control_custom">
                            ...
                            <option value="per_order" @selected(old('confirmer_payment_type', $profile->confirmer_payment_type) == 'per_order')>حسب الطلب</option>
                            <option value="monthly_salary" @selected(old('confirmer_payment_type', $profile->confirmer_payment_type) == 'monthly_salary')>راتب شهري</option>
                        </select>
                    </div>
                    ...
                    <div id="per_order_wrapper" style="display: none;">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" name="confirmer_payment_rate_per_order" class="form_control_custom" value="{{ old('confirmer_payment_rate_per_order', $profile->confirmer_payment_type == 'per_order' ? $profile->confirmer_payment_rate : '') }}">
                        </div>
                        <div class="mb-3">
                            <label for="confirmer_cancellation_rate" class="form-label">مكافأة الإلغاء</label>
                            <input type="number" step="0.01" name="confirmer_cancellation_rate" class="form_control_custom" value="{{ old('confirmer_cancellation_rate', $profile->confirmer_cancellation_rate) }}">
                        </div>
                    </div>
                    ...
                    <div id="monthly_salary_wrapper" style="display: none;">
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" name="confirmer_payment_rate_monthly" class="form_control_custom" value="{{ old('confirmer_payment_rate_monthly', $profile->confirmer_payment_type == 'monthly_salary' ? $profile->confirmer_payment_rate : '') }}">
                        </div>
                        <div class="mb-3">
                            <label for="salary_payout_day_input_edit" class="form-label">يوم استحقاق الراتب (1-28)</label>
                            <input type="number" name="salary_payout_day" id="salary_payout_day_input_edit" class="form_control_custom" min="1" max="28" value="{{ old('salary_payout_day', $profile->salary_payout_day) }}" aria-describedby="payout_day_helper_edit">
                            <small id="payout_day_helper_edit" class="form-text text-muted"></small>
                        </div>
                    </div>
                </div>

                <div class="d-grid mt-4">
                    <button type="submit" class="btn btn-lg" style="background-color: #6f42c1; color: white; font-weight: bold;">تحديث المستخدم</button>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const payoutDayInput = document.getElementById('salary_payout_day_input_edit');
    const payoutDayHelper = document.getElementById('payout_day_helper_edit');
